...
- The installer currently creates and removes only one launcher. Making `syndex` genuinely available
...
- `syndex` as the short interactive command.
...
### Manage `syndex` as an owned secondary launcher

Adding only a project script would make `syndex` available inside the prepared virtual environment,
but not through the launcher directory placed on the user's `PATH`. The installer will therefore
manage both the existing `code-indexing-mcp` launcher and the new `syndex` launcher.

The existing ownership rule remains mandatory: an unrelated executable named `syndex` must never be
...
syndex = "code_indexing_mcp.tui:main"
```

### 4. Install the `syndex` launcher
...
retaining `code-indexing-mcp` as the default. Add `syndex` as the managed secondary launcher,
targeting the generated `syndex` console script in the prepared environment.

Preserve the current safety rule: an unrelated existing executable named `syndex` must never be
...
include `syndex` in completion instructions, and flag a missing `syndex` launcher as repairable.
...
Verify POSIX and Windows paths for both launcher names, creation of `syndex`, replacement of an owned
stale alias, refusal to overwrite an unrelated `syndex`, and ownership-safe removal.

Edit `tests/test_installer_orchestrator.py`.

Verify successful creation of both launchers and warning behavior when only `syndex` cannot be
created.

Edit `tests/test_installer_uninstall.py`.

Verify uninstall removes both owned launchers and leaves a foreign `syndex` untouched.

Edit `tests/test_installer_tui.py`.

Verify command-access and completion panels advertise `syndex`, display its installation failure, and
...
Add a `Terminal UI` section showing `syndex` and `code-indexing-mcp tui`, key bindings, automatic
...
`syndex` launchers are managed.

## Acceptance Criteria

- A normal installation places both `code-indexing-mcp` and `syndex` in the configured launcher
  directory.
- Running `syndex` from inside a qualifying repository discovers or selects it and opens the TUI.
...
- Existing foreign executables named `syndex` are not overwritten or removed.
