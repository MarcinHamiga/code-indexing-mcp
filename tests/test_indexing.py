...
def test_rust_backfill_gains_occurrences_without_an_embedding_pass(tmp_path: Path) -> None:
    """The language-step version bump is the Rust backfill trigger.

    Rust files indexed before the bump carry coverage rows but zero
    occurrences (no structural extraction existed for them). After the
    version moves, the parse-only backfill must re-extract them --
    occurrences appear and `reference_extraction_ns` is real -- while never
    running an embedding pass.
    """
    root = tmp_path / "repo"
    root.mkdir()
...
def test_java_backfill_gains_occurrences_without_an_embedding_pass(tmp_path: Path) -> None:
    """The language-step version bump is the Java backfill trigger.

    Java files indexed before the bump carry coverage rows but zero
    occurrences (no structural extraction existed for them). After the
    version moves, the parse-only backfill must re-extract them --
    occurrences appear and `reference_extraction_ns` is real -- while never
    running an embedding pass.
...
def test_csharp_backfill_gains_occurrences_without_an_embedding_pass(tmp_path: Path) -> None:
    """The language-step version bump is the C# backfill trigger.

    C# files indexed before the bump carry coverage rows but zero
    occurrences (no structural extraction existed for them). After the
    version moves, the parse-only backfill must re-extract them --
    occurrences appear and `reference_extraction_ns` is real -- while never
    running an embedding pass.
