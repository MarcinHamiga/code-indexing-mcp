...
1. One row in `language_rules.py` (`LANGUAGE_RULES`), configuring node-type sets, reserved words, identifier validation, and import candidate resolution.
2. One handler entry in `_STRUCTURAL_RECORD_HANDLERS` in `src/code_indexing_mcp/extractor.py` pointing to the language's structural record extractor method, paired with its Tree-sitter query in `src/code_indexing_mcp/queries/<language>.scm`.
